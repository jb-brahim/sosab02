        ...
        return (
            <div className="flex h-screen items-center justify-center bg-background">
                <div className="flex flex-col items-center gap-4 animate-pulse">
                    ...
                    <p className="text-sm font-medium text-muted-foreground uppercase tracking-wider">{t("common.loading_system") || "Loading System"}</p>
                </div>
            </div>
        )
    }
            ...
            <div className="p-4 space-y-8 relative z-10">
                ...
                <div className="grid grid-cols-2 gap-3 animate-in delay-100">
                    <div className="glass-card rounded-xl p-4 flex flex-col justify-between h-28 relative overflow-hidden group">
                        ...
                        <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider z-10">{t("dashboard.active_projects") || "Active Projects"}</span>
                        <div className="flex items-end gap-2 z-10">
                            <span className="text-4xl font-bold font-display text-foreground">{projects.length}</span>
                            <span className="text-xs text-primary mb-1.5 font-medium">{t("dashboard.new") || "New"}</span>
                        </div>
                    </div>
                    <div className="glass-card rounded-xl p-4 flex flex-col justify-between h-28 relative overflow-hidden group">
                        <div className="absolute top-0 right-0 p-3 opacity-10 group-hover:opacity-20 transition-opacity">
                        ...
                        <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider z-10">{t("dashboard.total_sites") || "Total Sites"}</span>
                        <div className="flex items-end gap-2 z-10">
                            <span className="text-4xl font-bold font-display text-foreground">{projects.length}</span>
                            <span className="text-xs text-muted-foreground mb-1.5">{t("dashboard.locations") || "Locations"}</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="space-y-4 animate-in delay-200">
                    <div className="flex items-center justify-between px-1">
                        <h2 className="text-sm font-bold text-foreground uppercase tracking-widest flex items-center gap-2">
                            ...
                            {t("dashboard.current_operations") || "Current Operations"}
                        </h2>
                    </div>

                    {projects.length === 0 ? (
                        <div className="text-center py-12 glass rounded-2xl border-dashed">
                            <p className="text-muted-foreground text-sm">{t("dashboard.no_active_ops") || "No Active Operations"}</p>
                        </div>
                    ) : (
                        <div className="space-y-4" style={{ contentVisibility: 'auto' } as any}>
                            {projects.map((project, index) => (
                                <Link href={`/gerant/projects/${project._id}`} key={project._id} className="block group gpu will-change-transform">
                                    <div
