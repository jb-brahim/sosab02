...
export default function GerantReportsPage() {
    ...
    const handleDeleteReport = async () => {
        ...
        try {
            ...
            toast.error(t("reports.delete_failed") || "Failed to delete report")
        } finally {
            setDeleteId(null)
        }
    }
        ...
        <div className="p-4 space-y-4 pb-24">
            <div className="flex items-center gap-2 mb-4">
                ...
                <div>
                    ...
                    <p className="text-muted-foreground text-xs">{t("reports.description") || "View and manage reports"}</p>
                </div>
                <div className="ml-auto">
                    <Button size="sm" onClick={() => router.push('/gerant/reports/generate')}>
            ...
            <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
                {loading ? (
                    <div className="text-center py-12 text-muted-foreground animate-pulse font-medium">{t("reports.loading_reports") || "Loading..."}</div>
                ) : reports.length === 0 ? (
                    <div className="text-center py-20 text-muted-foreground bg-muted/20 rounded-2xl border border-dashed border-border/30">
                        ...
                        <p className="font-medium text-lg">{t("reports.no_reports") || "No reports found"}</p>
                        <p className="text-xs opacity-60">{t("reports.generate_new") || "Generate a new report to get started"}</p>
                    </div>
                ) : (
                                        ...
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div className="space-y-1">
                                                <h3 className="font-bold text-sm tracking-tight text-foreground/90">
                                                    {report.projectId?.name || t("common.unknown_project") || "Unknown Project"}
                                                </h3>
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className="text-[10px] text-muted-foreground uppercase font-black tracking-widest opacity-60">
                                                        {report.week && report.week !== 'CUSTOM' ? `${t("reports.week") || "Week"} ${report.week}` : t("reports.custom_range") || "Custom Range"}
                                                    </span>
                                                </div>
                                            </div>
                                        </div>
            ...
            <AlertDialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
                <AlertDialogContent className="rounded-2xl border-white/5 bg-background/95 backdrop-blur-xl">
                    <AlertDialogHeader>
                        <AlertDialogTitle className="font-display text-xl">{t("reports.delete_title") || "Delete Report"}</AlertDialogTitle>
                        <AlertDialogDescription>
                            {t("reports.delete_description") || "Are you sure you want to delete this report? This action cannot be undone."}
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter className="gap-2">
                        <AlertDialogCancel className="rounded-xl h-12">{t("common.cancel") || "Cancel"}</AlertDialogCancel>
